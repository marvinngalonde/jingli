...
export default function Transport() {
    ...
    const { data: vehicles = [], isLoading: vehiclesLoading } = useQuery({
        ...
        queryFn: () => api.get('/transport/vehicles').then(res => res.data || [])
    });
    ...
    const { data: stats = { totalVehicles: 0, activeRoutes: 0, studentsOnRoutes: 0 }, isLoading: statsLoading } = useQuery({
        queryKey: ['transportStats'],
        queryFn: () => api.get('/transport/stats').then(res => res.data)
    });

    const loading = vehiclesLoading || routesLoading || statsLoading;

    // Mutations
    const vehicleMutation = useMutation({
        mutationFn: (values: any) => editingId
            ? api.patch(`/transport/vehicles/${editingId}`, values)
            : api.post('/transport/vehicles', values),
        onSuccess: () => {
            notifications.show({ title: 'Success', message: editingId ? 'Vehicle updated' : 'Vehicle added', color: 'green' });
        ...
        onError: (err: any) => notifications.show({ title: 'Error', message: err.response?.data?.message || 'Failed to save vehicle', color: 'red' })
    });

    const routeMutation = useMutation({
        mutationFn: (values: any) => editingId
            ? api.patch(`/transport/routes/${editingId}`, values)
            : api.post('/transport/routes', values),
        onSuccess: () => {
            notifications.show({ title: 'Success', message: editingId ? 'Route updated' : 'Route added', color: 'green' });
        ...
        onError: (err: any) => notifications.show({ title: 'Error', message: err.response?.data?.message || 'Failed to save route', color: 'red' })
    });

    const deleteMutation = useMutation({
        mutationFn: ({ id, type }: { id: string, type: 'vehicle' | 'route' }) => api.delete(`/transport/${type === 'vehicle' ? 'vehicles' : 'routes'}/${id}`),
        onSuccess: (_, { type }) => {
            notifications.show({ title: 'Deleted', message: `${type === 'vehicle' ? 'Vehicle' : 'Route'} removed`, color: 'green' });
            queryClient.invalidateQueries({ queryKey: [type === 'vehicle' ? 'transportVehicles' : 'transportRoutes'] });
            queryClient.invalidateQueries({ queryKey: ['transportStats'] });
                                        ...
                                        <Table.Tr key={v.id}>
                                            ...
                                            <Table.Td>
                                                <Group gap="xs">
                                                    ...
                                                    <ActionIcon color="red" variant="subtle" loading={deleteMutation.isPending && deleteMutation.variables?.id === v.id} onClick={() => confirmDelete(v.id, 'vehicle', v.regNumber)}><IconTrash size={16} /></ActionIcon>
                                                </Group>
                                            </Table.Td>
                                        </Table.Tr>
                                    ))}
                                        ...
                                        <Table.Tr key={r.id}>
                                            ...
                                            <Table.Td>
                                                <Group gap="xs">
                                                    ...
                                                    <ActionIcon color="red" variant="subtle" loading={deleteMutation.isPending && deleteMutation.variables?.id === r.id} onClick={() => confirmDelete(r.id, 'route', r.name)}><IconTrash size={16} /></ActionIcon>
                                                </Group>
                                            </Table.Td>
                                        </Table.Tr>
                                    ))}
                    ...
                    <form onSubmit={vehicleForm.onSubmit(handleSaveVehicle)}>
                        <Stack>
                            ...
                            <Group justify="flex-end" mt="md">
                                ...
                                <Button type="submit" loading={vehicleMutation.isPending}>{editingId ? 'Update Vehicle' : 'Save Vehicle'}</Button>
                            </Group>
                        </Stack>
                    </form>
                ) : (
                    <form onSubmit={routeForm.onSubmit(handleSaveRoute)}>
                        <Stack>
                            ...
                            <Select
                                ...
                                data={vehicles.map((v: any) => ({ value: v.id, label: `${v.regNumber} - ${v.make} ${v.model}` }))}
                                clearable
                                searchable
                            ...
                            <Group justify="flex-end" mt="md">
                                <Button variant="default" onClick={closeDrawer}>Cancel</Button>
                                <Button type="submit" loading={routeMutation.isPending}>{editingId ? 'Update Route' : 'Save Route'}</Button>
                            </Group>
                        </Stack>
                    </form>
                )}
