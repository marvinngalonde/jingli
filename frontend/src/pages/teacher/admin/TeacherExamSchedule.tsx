...
import { Text, Badge, Group, Paper, Loader, Center, Card, ThemeIcon, SimpleGrid, Table, Avatar, Stack } from '@mantine/core';
...
export default function TeacherExamSchedule() {
    ...
    const loadExams = async () => {
        try {
            setLoading(true);
            const res = await api.get(`/exams?schoolId=${user?.schoolId || ''}`);
            setExams(res.data || []);
        } catch (error) {
            notifications.show({ title: 'Error', message: 'Failed to load exam schedule', color: 'red' });
        } finally {
            setLoading(false);
        }
    };
    ...
    const pastExams = exams.filter(e => new Date(e.startDate) < new Date());
                    ...
                    <Table.Tbody>
                        {data.map((exam) => (
                            <Table.Tr key={exam.id}>
                                ...
                                <Table.Td>
                                    <Badge variant="light" color="indigo" size="sm">{exam.classLevel?.name || '—'}</Badge>
                                </Table.Td>
                                <Table.Td>
                                    <Group gap="xs">
                                ...
                                <Table.Td>
                                    <Group gap="xs">
                                        <IconClock size={14} />
                                        <Text size="sm">{formatDate(exam.startDate)}</Text>
                                    </Group>
                                </Table.Td>
                                <Table.Td>
